...
from tempapps.boardWrite import myForm
...
def boardWrite(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'thanks.html', {'user':user, 'password':password, 'title':title,
                                                   'content':content, 'file':file}) # 템플릿 렌더링
    else:
        form = myForm()
    return render(request, 'board_wirte.html', {'form':form})